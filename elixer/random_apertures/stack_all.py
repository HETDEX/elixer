...
for i,file in enumerate(aper_files):
    ...
    #     t2 = None
    fluxd += list(t1['fluxd'])
    fluxd_err += list(t1['fluxd_err'])

    if t1 is not None:
        del t1

fluxd = np.array(fluxd)
fluxd_err = np.array(fluxd_err)

if True:

    # stack the apertures
    try:
        full_aper_flux_stack, full_aper_fluxe_stack, full_aper_grid, full_aper_contributions = SU.stack_spectra(
            fluxd,
            fluxd_err,
            np.tile(G.CALFIB_WAVEGRID, (len(fluxd), 1)),
            grid=G.CALFIB_WAVEGRID,
            avg_type=average,
            straight_error=False)
        full_aper_contributions = int(np.nanmean(full_aper_contributions))  # should actually be a constant
    except Exception as e:
        print(e)
        full_aper_flux_stack = np.full(len(G.CALFIB_WAVEGRID), np.nan)
        full_aper_fluxe_stack = np.full(len(G.CALFIB_WAVEGRID), np.nan)
        full_aper_contributions = 0




    # stack apertures by trimmed data
    ad = split_spectra_into_bins(fluxd, fluxd_err, sort=True, trim=0.666)
    i = 0
   ...
   #add to the table:
    T.add_row([
              #t1['ra'][0], t1['dec'][0], t1['shotid'][0], t1['seeing'][0],
               full_aper_flux_stack, full_aper_fluxe_stack,full_aper_contributions,
