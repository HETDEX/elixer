...
cloned from mcmc_pcygni

!!! should subtract continuum first !!! does not fit continuum (assumes zero)
...
class MCMC_Voigt:
    ...
    def model(self,x,theta):
        ...
        :return:
        """
        mu, sigma, A, mu2, sigma2, A2, ln_f = theta #note: not using ln_f here
        if (x is None) or (mu is None) or (sigma is None) or (mu2 is None) or (sigma2 is None):
            return None
        try:
            value = self.compute_model(x,mu,sigma,A,mu2,sigma2,A2)
    ...
    def run_mcmc(self):
       ...
       # self.delta_y = abs(self.initial_y)*0.2#,max(self.initial_peak,self.initial_peak_2)*0.1)
        ...
        initial_pos = [self.initial_mu, self.initial_sigma,  self.initial_A,
            ...
            if self.convergence_check is None or self.convergence_check <= 0:
                ...
                with warnings.catch_warnings():  # ignore the occassional warnings from the walkers (NaNs, etc that reject step)
                    ...
                    for i in range(0, self.main_run + self.convergence_check_step, self.convergence_check_step):
                        log.debug(
                            f"MCMC [{i}]. Burn in ({self.burn_in}), Max iter ({self.main_run}), convergence ({self.convergence_check})")
                        ...
                        if np.all(self.sampler.iteration > self.convergence_check * ac_t):
                            bConverged = True
                            log.info(
                                f"MCMC. All chains converged in ~ {i + self.convergence_check_step} steps at {self.convergence_check} scale.")
                            break  # stop sampling
        ...
        try:
            if self.samples is not None:
                ...
                fig = corner.corner(self.samples, labels=["$mu$", "$sigma$", "$A$", "$y$","$mu2$","$sigma2$","$A2$","f"],
                                    truths=[self.initial_mu, self.initial_sigma, self.initial_A,
