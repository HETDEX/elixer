    ...
    if count is not None:
        ...
        if sci_image:
            try:
                ...
                for h in sci_image: #headers
                    if 'MZP_AB' in h:
                        zero_point = float(h['MZP_AB'])
                        break
                    elif "PHOTZP" in h:  # this is in the deep "D3" imaging
                        zero_point = float(h['PHOTZP'])
                        break
        ...
        if len(keys) == 0: #we're done ... did not find any
            ...
            if "D3.?.fits" in keys:
                ...
                for k in keys:
                    ...
                    inside_ra = min((ra-self.Tile_Dict[adjusted_key]['RA_min']),(self.Tile_Dict[adjusted_key]['RA_max']-ra))
                    inside_dec = min((dec-self.Tile_Dict[adjusted_key]['Dec_min']),(self.Tile_Dict[adjusted_key]['Dec_max']-dec))

                    edge_dist = min(inside_dec,inside_ra)
                    #we want the tile with the largest minium edge distance

                    if edge_dist > max_dist and op.exists(self.Tile_Dict[adjusted_key]['path']):
                        max_dist = edge_dist
                        tile = k
    ...
    def get_single_cutout(self, ra, dec, window, catalog_image,aperture=None,error=None,do_sky_subtract=True,detobj=None):
        ...
        try:
            ...
            if G.APPLY_GALACTIC_DUST_CORRECTION and not self.imaging_already_dust_corrected:
                isowave = SU.filter_iso(catalog_image['filter'], None)
            ...
            cutout,pix_counts, mag, mag_radius,details = sci.get_cutout(ra, dec, error=error, window=window, aperture=aperture,
                                                                        ...
                                                                        detobj=detobj,dust_corr=dust_corr_mux,mag_corr=mag_corr_add)
            # don't need pix_counts or mag, etc here, so don't pass aperture or mag_func
            if cutout is not None:  # construct master cutout
                d['cutout'] = cutout
