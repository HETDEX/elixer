...
if "--gridsearch" in args:
    try:
        ...
        gridsearch_task_boost = int(gridsearch[0]/gridsearch[1]/2.0)
        base_time_multiplier = 1.0
    ...
    if queue == "skx-normal": #(192GB per node)
        ...
        if recover_mode:
            if neighborhood_only:
                ...
                MAX_TIME_PER_TASK = 1.10  # in recover mode, can bit more agressive in timing (easier to continue if timeout)
        else:
            if neighborhood_only:
                MAX_TIME_PER_TASK = 0.5
            else:
